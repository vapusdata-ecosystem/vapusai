...
func NewOIDCAuthenticator(opts *OIDCSecrets) (*OIDCAuthenticator, error) {
	_, opts.Domain = dmutils.TrailingSlash(opts.Domain, true, true)
	dmlogger.CoreLogger.Info().Msgf("Initializing OIDC Authenticator with domain: %v", opts.Domain)
	provider, err := oidc.NewProvider(context.Background(), opts.Domain)
	if err != nil {
