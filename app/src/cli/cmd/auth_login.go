...
func NewAuthLoginCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			log.Println(vapusGlobals.VapusPlatformClient)
...
func interactiveAuth(headlessLogin bool) error {
	...
	LoginParams, err := vapusGlobals.VapusPlatformClient.RetrieveLoginURL()
	if err != nil {
...
func generateIdToken(code, redirectUri string) error {
	...
	var err error
	accessToken, idToken, err := vapusGlobals.VapusPlatformClient.RetrieveAccessToken(code, redirectUri)
	if err != nil {
...
func (a *domainCliApp) callbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	http.Redirect(w, r, a.redirectURI, http.StatusSeeOther)
...
func localAuthServer() (net.Listener, *url.URL, error) {
	listener, err := net.Listen("tcp", "localhost:51101")
	if err != nil {
		return nil, nil, err
	}
