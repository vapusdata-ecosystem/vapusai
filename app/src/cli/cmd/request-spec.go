...
func specOps() {
	if listSpecs {
		vapusGlobals.VapusPlatformClient.ListPlatformSpec()
		return
	}
	if specName == "" {
		cobra.CheckErr("spec name is required")
	}
	err := vapusGlobals.VapusPlatformClient.GeneratePlatformSpec(viper.GetString(currentAccessToken), specName, format, withFakeData)
	if err != nil {
		cobra.CheckErr(err)
	}
}
