...
func NewRootCmd() *cobra.Command {
	...
	rootCmd.PersistentFlags().StringVar(&action, "action", "", "Action for the platform that should be executed on current resource with params in a file")
	rootCmd.PersistentFlags().StringVar(&file, "file", "", "File containing the parameters for the action")
	rootCmd.PersistentFlags().StringVar(&search, "search", "", "Search query for the resource")
	...
	rootCmd.AddCommand(NewClearCmd(), NewConfigCmd(), NewExplainCmd(), NewAuthCmd(), NewGetCmd(), NewConnectCmd(),
		NewDescribeCmd(), NewRequestSpecCmd(), NewActCmd(), NewSearchmd())
	return rootCmd
}
...
func initCurrentContextInstance() {
	...
	if currentContextParams == nil {
		...
		if vapusGlobals.VapusPlatformClient == nil {
			...
			vapusGlobals.VapusPlatformClient = client
